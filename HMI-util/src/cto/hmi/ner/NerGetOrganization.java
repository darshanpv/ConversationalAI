...
public class NerGetOrganization {
	public static List<String> ORGANIZATION = new ArrayList<String>();
	...
	public static String get() {
				...
				for (int i = 0; i < elementsList.getLength(); i++) {
					if (elementsList.item(i).hasChildNodes()) {
						...
						for (int i2 = 0; i2 < tagsList.getLength(); i2++) {
							...
							if (tagNode.getNodeName().matches("ORGANIZATION"))
								ORGANIZATION.add(tagNode.getTextContent());
		...
		for (int i = 0; i < ORGANIZATION.size(); i++) {
			output = output + "|" + ORGANIZATION.get(i);
		}
		ORGANIZATION.clear();
		return output;
	}
